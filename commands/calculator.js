...
module.exports = {
	...
	async execute({ message, embed, args }) {
		...
		let row = [];
		let text = ["C", "(", ")", "/", "7", "8", "9", "*", "4", "5", "6", "-", "1", "2", "3", "+", ".", "0", "00", "="];
		let current = 0;
		...
		message.channel.send({embeds: [embed], components: row}).then(msg => {
			...
			const filter = (x) => x.user.id === message.author.id && x.customId === "calc" + val;
			const collect = msg.createMessageComponentCollector({filter, time});
			...
			for(let txt of text) {
				...
				setTimeout(() => {
					embed1.setDescription("Tu tiempo para usar la calculadora ha acabado")
					.setColor("#dc3545");
